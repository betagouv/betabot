...
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates curl git jq tree procps && rm -rf /var/lib/apt/lists/*
...
COPY src ./src
COPY get-data.sh build-embeddings.ts build-db.ts fetch-docs.ts fetch-messagerie-docs.ts fetch-wttj.ts fetch-choisirleservicepublic.ts create-report.ts ./
